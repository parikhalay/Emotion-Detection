...
def check_dataset_balance(csv_file):
    dataset = load_custom_dataset(csv_file)
    class_counts = {}

    for _, gender, age in dataset:
        class_key = (gender, age)
    ...
    for class_key, count in class_counts.items():
        print(f"Class {class_key}: {count} instances")
...
if __name__ == '__main__':
    ...
    model_path = 'savemodel/fold/facial_recognition_fold_7.pth'
    csv_file = 'Part2_test_labels.csv'

    check_dataset_balance(csv_file)
